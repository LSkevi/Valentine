// @ts-nocheck
// LEGACY BRIDGE — this is the original game.js copied as-is.
// Modules are being extracted from here incrementally.
...
function applyBgStyle() {
  ...
  if (!canvas) return;
  if (typeof BG_GRADIENTS === "undefined" || !BG_GRADIENTS) return; // guard: not yet initialized
...
// In module scope, functions aren't global. These are needed by index.html onclick attributes.
(window as any).closeModal = closeModal;
(window as any).collectSeed = collectSeed;
(window as any).buyPacket = buyPacket;
(window as any).buyWaterUpgrade = buyWaterUpgrade;
(window as any).buyPot = buyPot;
(window as any).openHelpGuide = openHelpGuide;
(window as any).helpPage = helpPage;
(window as any).selectPktType = selectPktType;
// Debug helpers
(window as any).G = G;
(window as any).saveG = saveG;
(window as any).FLOWERS = FLOWERS;
(window as any).updateHUD = updateHUD;
(window as any).renderGarden = renderGarden;
(window as any).renderTray = renderTray;
(window as any).renderDrops = renderDrops;
(window as any).renderWeather = renderWeather;
(window as any).startAmbientMusic = startAmbientMusic;
(window as any).stopAmbientMusic = stopAmbientMusic;
(window as any).openBreedGuide = openBreedGuide;
